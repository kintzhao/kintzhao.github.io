...
```bash
# 使用 conda 创建隔离环境
conda create -n deepseek_distill python=3.10 -y
...
# 或使用预设配置文件 accelerate_config.yaml
```
...
```bash
# 拉取 NVIDIA PyTorch 官方镜像（内置 CUDA 12.1）
...
# 容器内安装额外依赖
pip install transformers trl accelerate deepspeed vllm
```
