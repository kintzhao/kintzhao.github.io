---
title: DeepSeek R1 从零训练模型数据-模型蒸馏技术实战综述
source: doc0_综述.docx
---
...
1. **数据准备**：从 DeepSeek R1 教师模型采样高质量 CoT（思维链）数据，经过清洗、过滤、格式化
2. **环境搭建**：配置 CUDA、Python 依赖、Transformers、TRL 等核心训练框架
3. **蒸馏训练**：采用监督微调（SFT）或 KL 散度蒸馏两种策略对学生模型进行训练
4. **评估验证**：在 MATH-500、AIME、LiveCodeBench 等基准上量化蒸馏效果
5. **部署推理**：使用 vLLM 或 llama.cpp 对蒸馏后模型进行加速部署
...
2. Hinton, G., Vinyals, O., & Dean, J. Distilling the Knowledge in a Neural Network. NeurIPS 2014.
3. HuggingFace TRL 官方文档: https://huggingface.co/docs/trl
4. DeepSeek 官方 GitHub: https://github.com/deepseek-ai/DeepSeek-R1
