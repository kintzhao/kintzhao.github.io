...
冷启动 SFT：用少量高质量 CoT 数据（数千条）微调基础模型，帮助模型建立输出格式。
推理导向 RL：以 GRPO 在大规模数学/代码问题上进行强化学习，激发深度推理。
拒绝采样 + SFT：从 RL 模型采样高质量样本，过滤低质答案，混合通用数据再次 SFT。
对齐 RL：加入人类偏好奖励信号，提升模型安全性与有用性。

## 3.5 奖励设计原则
DeepSeek-R1 使用基于规则（Rule-based）的奖励，避免奖励模型的幻觉问题：
准确性奖励：数学题答案正确得 +1，错误得 -1；代码题通过测试用例得分。
格式奖励：输出包含 <think>...</think> 结构得额外分，鼓励显式推理。
...
| # GRPO 核心思想伪代码 for question in math_dataset:     # 对同一问题采样 G 个输出     outputs = [model.generate(question) for _ in range(G)]     # 计算每个输出的奖励（基于规则：答案正确=+1, 格式正确=+0.1）     rewards = [reward_fn(o, answer) for o in outputs]     # 归一化奖励，计算相对优势     advantages = normalize(rewards)     # 使用 PPO-clip 风格更新策略     loss = -mean(advantages * log_prob(outputs))     optimizer.step(loss) |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
